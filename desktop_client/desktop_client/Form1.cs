    ...
    public partial class Form1 : Form
    {
        ...
        private async void buttonLoadDepartments_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    c.Students = await service.GetStudentsByCourseId(c.Id);
                }
                ...
                else
                {
                    foreach (var dept in departments)
                        listBoxDepartments.Items.Add($"{dept.Id} - {dept.Name}");
                }
            }
            catch (Exception ex)
            {
            ...
            if (index >= 0 && index < departments.Count)
            {
                ...
                else
                {
                    foreach (var course in filteredCourses)
                        listBoxCourses.Items.Add($"{course.Id} - {course.Name}");
                }
            }
        }
            ...
            if (index >= 0 && index < filteredCourses.Count)
            {
                ...
                else
                {
                    foreach (var student in selectedCourse.Students)
                        listBoxStudents.Items.Add($"{student.Id} - {student.Name} ({student.Email})");
                }
            }
        }
